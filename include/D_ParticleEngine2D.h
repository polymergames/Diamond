...
struct ParticleSystem2DConfig {
  ...
  // Angle (range) in degrees towards which a particle is emitted from its
  // emission point.
  ...
  // If negative, particle will travel in the opposite direction of its emission
  // angle.
  ...
  // If animateColor = true, each particle's color will change smoothly
  // through its lifetime from birthColor to deathColor.
  bool animateColor = false;
  RGB minBirthColor = RGB{255, 255, 255};
  RGB maxBirthColor = RGB{255, 255, 255};
  RGB minDeathColor = RGB{255, 255, 255};
  RGB maxDeathColor = RGB{255, 255, 255};

  // If animateAlpha = true, each particle's transparency will change smoothly
  // through its lifetime from birthAlpha to deathAlpha (out of 255).
  bool animateAlpha = false;
  uint8_t minBirthAlpha = 255;
  uint8_t maxBirthAlpha = 255;
  uint8_t minDeathAlpha = 255;
  uint8_t maxDeathAlpha = 255;
...
public:
  ...
  Particle2D(Particle2D &&other)
      : transform(other.transform), renderComponent(other.renderComponent),
        data(other.data), age(other.age), lifeTime(other.lifeTime),
        velocity(other.velocity), acceleration(other.acceleration),
        angularSpeed(other.angularSpeed), animateScale(other.animateScale),
        deathScale(other.deathScale), animateColor(other.animateColor),
        deathColor(other.deathColor), animateAlpha(other.animateAlpha),
        deathAlpha(other.deathAlpha) {
  ...
  Particle2D &operator=(Particle2D &&other) {
    if (this != &other) {
      ...
      animateAlpha = other.animateAlpha;
      deathAlpha = other.deathAlpha;
  ...
  /**
   * The particle system will only modify effects on a render component, and
   * will not change the texture itself. Therefore, the texture can be
   * externally set and animated.
   */
  ...
  // colors
  bool animateColor = false;
  RGB deathColor = RGB{255, 255, 255};
  bool animateAlpha = false;
  uint8_t deathAlpha = 255;
};

/**
 * An object of this class emits particles from its origin transform.
 * It will do fancy things for you.
 * User is responsible for allocating, storing, updating, and destroying the
 * emitted particles.
 */
...
public:
  ...
  using SpawnParticleFunc =
      std::function<Particle2D &(const ParticleSystem2DConfig &config)>;

  using InitParticleFunc = std::function<void(
   ...
   * a reference to it.
   *
   * The optional onInitParticle function is called after a particle is
   * initialized and setup for emission, and can be used to customize the
   * particle and perform whatever other mischief the user desires.
   */
   ...
   * after each time interval (as configured in the emitter's
   * ParticleSystem2DConfig).
   */
  void update(tD_delta delta) override;
  ...
  // the particle emitter's transform (which is the source of all of its
  // particles in a scene graph)
