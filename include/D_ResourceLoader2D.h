...
namespace Diamond {
/**
 * These functions construct resource definition structs from configuration
 * tables
 ...
 * (which could, for ex., be written to a config file using
 * ConfigLoader::write).
 */
class ResourceLoader2D {
 public:
  bool loadRenderDef(const ConfigTable &config, RenderDef2D &renderDef,
                     std::string &texturePath, Vector2<tD_real> &scale) const;

  bool loadAABBDef(const ConfigTable &config, AABBDef2D &aabb) const;

  bool loadCircleDef(const ConfigTable &config, CircleDef &circle) const;
  ...
  ConfigTable genRenderConfig(
      ...
      const Vector2<tD_real> &scale = Vector2<tD_real>(1, 1)) const;
