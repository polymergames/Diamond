...
void Diamond::QuantumBodyDeleter::free(void *body) const {
    QuantumBody2D *qbody = static_cast<QuantumBody2D*>(body);
...
void Diamond::QuantumColliderDeleter::free(void *ptr) const {
    Collider2D *collider = static_cast<Collider2D*>(ptr);
...
Diamond::DumbPtr<Diamond::Rigidbody2D> Diamond::QuantumWorld2D::makeRigidbody(DTransform2 &transform) {
    ...
    return DumbPtr<Rigidbody2D>(body, &m_bodyDeleter);
}


Diamond::DumbPtr<Diamond::AABBCollider2D> Diamond::QuantumWorld2D::makeAABBCollider(
    ...
    if (qbody) {
        ...
        if (aabb)
            return DumbPtr<AABBCollider2D>(m_aabbPool.make(col, aabb),
                                           &m_colliderDeleter);
    }
    return nullptr;
}
...
Diamond::DumbPtr<Diamond::CircleCollider> Diamond::QuantumWorld2D::makeCircleCollider(
    const Rigidbody2D *body,
    void *parent,
    const std::function<void(void *other)> &onCollision,
    tD_pos radius,
    ...
    if (qbody) {
        ...
        if (circle)
            return DumbPtr<CircleCollider>(m_circlePool.make(col, circle),
                                           &m_colliderDeleter);
    }
    return nullptr;
}
...
Diamond::DumbPtr<Diamond::PolyCollider> Diamond::QuantumWorld2D::makePolyCollider(
    const Rigidbody2D *body,
    void *parent,
    const std::function<void(void *other)> &onCollision,
    ...
    if (qbody) {
        ...
        if (poly)
            return DumbPtr<PolyCollider>(m_polyPool.make(col, poly),
                                         &m_colliderDeleter);
    }
    return nullptr;
}
